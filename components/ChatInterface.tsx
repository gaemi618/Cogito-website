...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ animusId, onBack }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black text-gray-100" style={containerStyle}>
      ...
      {/* Header */}
      <header className="relative z-10 flex items-center p-4 border-b border-white/10 glass-panel backdrop-blur-md">
        <button onClick={onBack} className="p-2 hover:bg-white/10 rounded-full transition-colors text-white">
          <ArrowLeft size={24} />
        </button>
        <div className="flex items-center ml-4">
          <div className="w-10 h-10 rounded-full overflow-hidden border border-white/30 mr-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-300">{character.locationName}</p>
          </div>
        </div>
      </header>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} items-end`}>
              {!isUser && (
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-2 border border-${character.themeColor} bg-black/50 overflow-hidden shrink-0`}>
                  <img src={character.profileImage} alt="Avatar" className="w-full h-full object-cover" />
                </div>
              )}
              <div 
                className={`max-w-[80%] p-3 rounded-2xl whitespace-pre-wrap leading-relaxed shadow-lg ${
                  isUser 
        ...
        {isLoading && (
          <div className="flex justify-start items-center">
             <div className="w-8 h-8 rounded-full flex items-center justify-center mr-2 overflow-hidden shrink-0 opacity-70">
                <img src={character.profileImage} alt="Thinking" className="w-full h-full object-cover grayscale" />
             </div>
      ...
      <div className="relative z-10 p-4 glass-panel border-t border-white/10">
        <form onSubmit={handleSendMessage} className="flex gap-2 max-w-4xl mx-auto">
          <input
            ...
            className="flex-1 bg-black/40 border border-gray-600 rounded-full px-6 py-3 text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all placeholder-gray-400 backdrop-blur-sm"
          />
          <button 
            type="submit"
            disabled={isLoading || !inputText.trim()}
            className="p-3 bg-white/10 hover:bg-white/20 rounded-full transition-colors disabled:opacity-50 text-white border border-white/10"
          >
            <Send size={20} />
          </button>
        </form>
